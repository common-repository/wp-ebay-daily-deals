...
Plugin URI: http://www.whitefoam.net/wp-ebay-daily-deals-wordpress-plugin/
    ...
    */
    function widget($args, $instance){
      ...
      //echo '<div style="text-align:center;padding:10px;">' . $lineOne . '<br />' . $lineTwo . "</div>";
		$xmlurl = "http://deals.ebay.com/feeds/new/xml";
...
$raw_xml = str_replace("ns2:", "", $raw_xml);
...
//print_r($resp);
		...
		if ($resp) {
			if($catagory=="all" || $catagory==""){
				...
				foreach($resp->DealModule->Item as $item) {
					...
					$results[$n][SmallPictureURL]= $item->Image140URL;
					$results[$n][PictureURL]= $item->ImageURL;
					$results[$n][Picture175Url]= $item->Image300URL;
					$results[$n][DealURL]= $item->DealURL;
					$results[$n][ConvertedCurrentPrice]= $item->SalePrice;
					$results[$n][MSRP]= $item->ListPrice;
		...
		?>
		  <table style="text-align: left; <?php echo $userfont; ?> font-size: <?php echo $textsize;?>px; width: 100%;" border="0" cellpadding="0" cellspacing="0">
			<tr>
			  <td>
			  <div style="font-weight: bold; padding-bottom: 2px; line-height: 14px;"><a style="color: rgb(130, 130, 130); text-decoration: none;" href="<?php echo $results[$j][pURLhide];?>" target="_blank" rel="nofollow">
			  <img alt="<?php echo $results[$j][Title];?>" style="border: 0px solid; float: left;margin-right: 3px; margin-top: 2px;" src="<?php echo $results[$j][SmallPictureURL];?>" width="75px" >			 
		<?php echo $results[$j][Title];?></a></div>
		 ...
		 style="text-decoration: line-through;"><?php echo $results[$j][MSRP];?></span>
			...
			echo "<tr><td colspan='2' rowspan='1'><hr style='width: 100%;height: 1px;margin: 10px;'></td></tr>";
			}
			?>
			</table>
		...
		?>
			...
			<tr>
			  ...
			  if($linkback=="yes"){
			  ?>
			  <a style="color: rgb(102, 102, 102); text-decoration: none; font-size: <?php echo $textsize-2;?>px;" href="http://www.whitefoam.net/wp-ebay-daily-deals-wordpress-plugin/" target="_blank">
